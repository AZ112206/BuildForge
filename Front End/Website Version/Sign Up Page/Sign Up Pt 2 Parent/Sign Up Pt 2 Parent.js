document.addEventListener("DOMContentLoaded", () => {
  ...
  function initCustomSelect(selectRoot) {
    ...
    function syncOptionState(nextValue) {
      ...
      if (activeOption) {
        valueText.textContent = activeOption.textContent.trim();
  ...
  function toFlagEmoji(countryCode) {
    ...
    const first = countryCode.charCodeAt(0) - 65 + 127462;
    const second = countryCode.charCodeAt(1) - 65 + 127462;
  ...
  function getCountryOptions() {
    ...
    const options = allCountryCodes
      ...
      .map((option) => ({
        value: option.value,
        label: `${toFlagEmoji(option.value)} ${option.name}`
      }));
