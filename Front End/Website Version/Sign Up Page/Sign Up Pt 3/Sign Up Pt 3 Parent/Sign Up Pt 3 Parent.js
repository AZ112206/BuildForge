document.addEventListener("DOMContentLoaded", () => {
  ...
  const confirmPasswordInput = document.getElementById("parent-confirm-password");
  const passwordStrengthValue = document.getElementById("parent-password-strength-value");
  const passwordMatchError = document.getElementById("parent-password-match-error");
  ...
  function updatePasswordStrengthMeter() {
    ...
    if (passwordStrengthValue) {
      passwordStrengthValue.textContent = password ? label : "–";
      passwordStrengthValue.style.color = password ? strengthColors[Math.max(score - 1, 0)] : "var(--text-muted)";
    }
  }
  ...
  function updatePasswordMatchState() {
    ...
    passwordMatchError.style.display = isMismatch ? "block" : "none";
  ...
  if (backBtn) {
    backBtn.addEventListener("click", () => {
      window.location.href = "../../Sign Up Pt 2/Sign Up Pt 2 Parent/Sign Up Pt 2 Parent.html";
    });
  }
  ...
  form.addEventListener("submit", (event) => {
    ...
    showSignupSuccess("parent", parentData);
  });
});
