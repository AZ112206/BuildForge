document.addEventListener("DOMContentLoaded", () => {
  ...
  const draftKey = "makerpodsStudentAccountDraft";
  ...
  function applyAppearance() { const theme = localStorage.getItem("makerpodsTheme") || (window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light"); const mode = localStorage.getItem("makerpodsSurfaceMode") === "solid" ? "solid" : "transparent"; htmlElement.setAttribute("data-theme", theme); htmlElement.setAttribute("data-surface-mode", mode); themeIcon.innerHTML = theme === "dark" ? SUN_SVG : MOON_SVG; transparencyIcon.innerHTML = mode === "transparent" ? CLEAR_SQ_SVG : SOLID_SQ_SVG; }
  ...
  backBtn.addEventListener("click", () => { goTo("../../Sign Up Pt 2a/Sign Up Pt 2a Student/Sign Up Pt 2a Student.html"); });
  form.addEventListener("submit", (event) => {
    event.preventDefault();
    sessionStorage.setItem("makerpodsUserRole", "student");
    sessionStorage.setItem("makerpodsStudentAccountData", JSON.stringify({ email: email.value.trim(), role: "student" }));
